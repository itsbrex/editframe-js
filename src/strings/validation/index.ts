...
export const ValidationErrorText = {
  ...
  TWO_TRANSITIONS_REQUIRED: (type: string): string =>
    `Only one transition of type \\${chalk.white(
      type
    )}\\ has been added. There must be at least two transitions added in order to interpolate between the transition values`,
}
